...
## Decision: Consolidated Transliteration-Aware Language Detection
**Date**: 2026-07-14
**Status**: Accepted → **Implemented** (Epic 6)
**Context**: Four independent language detection implementations existed (gateway franc-min, PersonaPlex script counting, admin-api langdetect, legacy fasttext). Transliterated Uzbek text (e.g., "Assalomu alaykum", "rahmat") was misdetected as English. Keyword sets were small (~40 words each). No shared detection code.
**Decision**: Create `language-detection.ts` as the single source of truth for the NestJS gateway. Expanded keyword sets to 150+ Latin and 80+ Cyrillic words including common verb forms, case endings, and transliterated patterns. Added 3-tier confidence: definitive signals (okina, Uzbek Cyrillic chars → 0.95), keyword matching (2+ matches → 0.85-0.9), transliterated indicators (1+ match → 0.7). franc-min serves as fallback when keyword signals are weak. PersonaPlex endpoint keyword sets also expanded.
**Consequences**: Transliterated Uzbek text now correctly detected as `uz` instead of `en`. Single shared module eliminates code duplication. Detection accuracy improved for short texts and common phrases. franc-min fallback preserved for edge cases.
