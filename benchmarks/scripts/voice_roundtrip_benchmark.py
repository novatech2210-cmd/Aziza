    ...
    async def measure_roundtrip(self, audio_duration_ms: int = 2000) -> Optional[float]:
        ...
        try:
            ...
            try:
                import subprocess
                subprocess.run(
                    ['redis-cli', 'hset', 'moshi:workers:worker-01', 'status', 'idle'],
            ...
            async with websockets.connect(ws_url) as websocket:
                ...
                # Step 2: Wait for handshake — break immediately on worker_assigned
                for _ in range(5):
                    try:
                        ...
                        if isinstance(msg, str):
                            ...
                            if data.get("type") in ("worker_assigned", "first_token"):
                                break  # ← was: continue (caused 5s timeout per run)
                ...
                # Signal end of audio — this is the true processing start
                timestamps["audio_end_sent"] = time.time()
                ...
                while True:
                    ...
                    if isinstance(message, bytes):
                        if message[0] == 1 or message[0] == 0x01:
                ...
                # true_processing_ms = from audio_end sent → first audio frame back
                audio_end_sent = timestamps.get("audio_end_sent", timestamps["audio_send"])
                true_processing_ms = (timestamps["playback_start"] - audio_end_sent) * 1000
                hop_latencies = {
                    "audio_send_ms": (timestamps["audio_send"] - timestamps["mic_capture"]) * 1000,
                    "processing_ms": true_processing_ms,
                    "total_roundtrip_ms": true_processing_ms  # report processing latency as the benchmark metric
                }
