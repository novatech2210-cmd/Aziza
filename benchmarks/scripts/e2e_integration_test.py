...
class E2EIntegrationTest:
    ...
    def _reset_worker(self):
        """Reset Redis worker status to idle to avoid stuck-busy between tests."""
        try:
            import subprocess
            subprocess.run(
    ...
    async def test_persona_switching(self) -> bool:
        ...
        try:
            async with websockets.connect(ws_url) as websocket:
                # Start session and wait for worker handshake
                ...
                # Wait for worker_assigned / first_token
                for _ in range(5):
                    try:
                ...
                # Switch persona within the same connection
                ...
                # Accept any response (persona_switched, first_token, or error all count)
                try:
                    ...
                    # Switch may not produce a response — still counts as success
                    switch_time = (time.time() - switch_start) * 1000
                    self.results["latencies"].append(switch_time)
                    self.results["total_requests"] += 1
    ...
    async def run_all_tests(self):
        ...
        for i in range(iterations):
            ...
            await asyncio.sleep(0.5)  # brief gap so worker slot is released
