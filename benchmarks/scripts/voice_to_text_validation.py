...
class VoiceToTextValidation:
    ...
    async def transcribe_audio(self, audio_data: bytes, language: str) -> Optional[Dict]:
        ...
        try:
            ...
            try:
                import subprocess
                subprocess.run(
                    ['redis-cli', 'hset', 'moshi:workers:worker-01', 'status', 'idle', 'sessions', '0'],
            ...
            async with websockets.connect(ws_url) as websocket:
                ...
                # Step 2: Wait for handshake (worker_assigned confirms gateway is ready)
                for _ in range(5):
                    try:
                        msg = await asyncio.wait_for(websocket.recv(), timeout=5.0)
                        if isinstance(msg, str):
                            data = json.loads(msg)
                            if data.get("type") in ("worker_assigned", "first_token"):
                                break  # handshake complete — proceed to send audio
                ...
                # Wait for transcript (or equivalent signal from gateway)
                transcript = None
                dropped = 0
                while True:
                    if time.time() - start_time > 6.0:
                        if transcript is not None:
                            break
                        raise asyncio.TimeoutError("Overall transcription timeout")
                    message = await asyncio.wait_for(websocket.recv(), timeout=10.0)

                    # Binary 0x02 frame = text token from voice gateway
                    if isinstance(message, bytes):
                        if len(message) > 1 and message[0] == 0x02:
                            try:
                                ...
                                pass
                        continue  # keep reading until playback_done
                    ...
                    if data.get("type") == "transcript":
                        ...
                        break
                    elif data.get("type") in ("playback_done", "done"):
                        # Gateway finished streaming — transcript captured from 0x02 frame
    ...
    async def run_validation(self):
        ...
        print(f"Running Voice→Text Validation...", flush=True)
        print(f"Target: <{self.target_s}s", flush=True)
        print(f"Languages: {', '.join(self.languages)}", flush=True)
        
        # For each language, test multiple samples
        samples_per_language = 10
        
        for language in self.languages:
            print(f"\n  Testing {language}...", flush=True)
            
            for i in range(samples_per_language):
                print(f"    Sample {i+1}/{samples_per_language}", flush=True)
                ...
                if result:
                    ...
                    print(f"      Latency: {result['latency_s']:.2f}s, WER: {result['wer']:.2f}", flush=True)
                else:
