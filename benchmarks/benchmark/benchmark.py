...
async def run_session(redis_url, session_id, input_pcm, start_delay=0):
    await asyncio.sleep(start_delay)
    r = redis.from_url(redis_url)
    pubsub = r.pubsub()
    await pubsub.subscribe(f"session:{session_id}:playback")
    ...
    output_file = open(f"results/session_{session_id}_output.raw", "wb")
    text_log = open(f"results/session_{session_id}_text.log", "w")
    ...
    async def send_audio():
        for i in range(0, len(input_pcm), CHUNK_SIZE):
            ...
            await r.publish(f"session:{session_id}:audio", chunk)
...
async def monitor_gpu(duration):
    start = time.time()
    metrics_log = []
    with open("results/gpu_utilization.log", "w") as f:
        f.write("Timestamp,GPU_Util,Mem_Util,VRAM_Used,VRAM_Total\n")
        while time.time() - start < duration:
            m = get_gpu_metrics()
            if m:
                log_line = f"GPU Util: {m['gpu_util']}%, Mem Util: {m['mem_util']}%, VRAM: {m['mem_used_mb']}/{m['mem_total_mb']} MB"
                logger.info(log_line)
                f.write(f"{time.time()},{m['gpu_util']},{m['mem_util']},{m['mem_used_mb']},{m['mem_total_mb']}\n")
                f.flush()
...
async def main():
    ...
    for i in range(args.sessions):
        session_id = f"{i}_{uuid.uuid4().hex[:4]}"
        tasks.append(run_session(args.redis, session_id, input_pcm, start_delay=i*0.5))
    ...
    with open("results/benchmark_summary.txt", "w") as f:
        f.write(f"Target Concurrent Sessions: {args.sessions}\n\n")
        for i, r in enumerate(results):
            summary = f"Session {i}: TTFT={r['ttft']:.2f}ms, Avg Latency={r['avg_latency']:.2f}ms, Max Pause={r['max_pause']:.2f}ms"
            logger.info(summary)
            f.write(summary + "\n")
