...
class Checkpointer:
    ...
    def save_checkpoint(
        ...
        # Gather optimizer state (collective op — all ranks must call)
        optim_state = None
        if self.optimizer is not None:
            if get_world_size() > 1:
                optim_state = FullyShardedDataParallel.full_optim_state_dict(
        ...
        if self.rank == 0:
            ...
            train_state_dict = {
                "train_state": {
                    ...
                    "n_seen_tokens": self.state.n_seen_tokens,
                },
            }
            if optim_state is not None:
                train_state_dict["optimizer"] = optim_state
            if self.scheduler is not None:
                train_state_dict["scheduler"] = self.scheduler.state_dict()
            torch.save(train_state_dict, tmp_dst / "train_state.pt")
    ...
    @staticmethod
    def load_train_state(ckpt_dir: Path) -> dict | None:
        """Load train_state.pt from a checkpoint directory, if it exists."""
        # ckpt_dir may be .../checkpoint_000400/consolidated or .../checkpoint_000400
        for candidate in [ckpt_dir / "train_state.pt", ckpt_dir.parent / "consolidated" / "train_state.pt"]:
            if candidate.exists():
