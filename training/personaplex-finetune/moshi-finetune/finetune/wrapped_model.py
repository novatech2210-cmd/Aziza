...
def get_fsdp_model(
    args: TrainArgs, checkpointer_info: CheckpointInfo, resume_lora_path: str | None = None
    ...
    if get_rank() == 0:
        ...
        if args.lora.enable and not args.full_finetuning:
            ...
            if resume_lora_path:
                logger.info(f"Loading resumed LoRA weights from {resume_lora_path}")
                lora_state = safetensors.torch.load_file(resume_lora_path)
                lora_state = {k: v.to(param_dtype) for k, v in lora_state.items()}
                model.load_state_dict(lora_state, strict=False, assign=True)
    ...
    if args.lora.enable and not args.full_finetuning:
        ...
        for name, param in model.named_parameters():
            if "lora" in name:
                # Optionally freeze depformer LoRA — keeps pretrained audio
                # generation intact (lora_B is initialized to zeros so frozen
                # depformer LoRA has zero contribution).
                if args.lora.skip_depformer and "depformer" in name:
        ...
        if n_skipped > 0:
            main_logger_info(
                f"Froze {n_skipped} depformer LoRA params (skip_depformer=True)"
            )
    else:
