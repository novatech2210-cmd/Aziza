...
export const useUserAudio = ({
  ...
  const startRecordingUser = useCallback(async () => {
    ...
    if (mediaStream) {
      ...
      // Pass sourceNode so opus-recorder uses the SAME mic stream as the analyser.
      // Without this, it calls getUserMedia separately and may pick a different device
      // (e.g. when AirPods are connected, the second call can resolve to built-in mic).
