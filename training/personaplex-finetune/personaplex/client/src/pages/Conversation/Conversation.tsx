...
const buildURL = ({
  ...
  const isGemini = modelQuery === "gemini";
  const endpoint = isGemini ? "/api/gemini-chat" : "/api/chat";
  const url = new URL(`${wsProtocol}://${newWorkerAddr}${endpoint}`);

  // Gemini only needs text_prompt and greeting; Moshi needs all params
  url.searchParams.append("text_prompt", params.textPrompt.toString());
  if (params.greeting) {
    url.searchParams.append("greeting", params.greeting);
  }
  ...
  if (!isGemini) {
    ...
    if (modelQuery) {
      url.searchParams.append("model", modelQuery);
    }
  }
...
export const Conversation:FC<ConversationProps> = ({
  ...
  useEffect(() => {
    worklet.current?.port.postMessage({ type: "silenceThreshold", value: silenceThreshold });
  }, [silenceThreshold, worklet]);
  ...
  return (
    ...
    <div className="main-grid flex-1 w-full">
        ...
        <ChatContext.Provider value={chat}>
          ...
          <div className="scrollbar player-text rounded-lg border border-gray-100 bg-white p-3" ref={textContainerRef}>
            <TextDisplay containerRef={textContainerRef}/>
          </div>
          <div className="player-stats">
            <ServerAudioStats getAudioStats={getAudioStats} />
          </div>
        </ChatContext.Provider></MediaContext.Provider>}
        </div>
      </div>
    </SocketContext.Provider>
  );
