#!/usr/bin/env python3
"""
train_uzbek.py — Aziza Uzbek QLoRA fine-tuning script
Novatech | AZIZA-BUILD-PLAN-PH2-PH3

Trains a QLoRA adapter on PersonaPlex-7B for Uzbek Latin and Cyrillic text generation.
Run in screen/tmux — training takes 12–18 hours on H100 SXM.

Usage:
    python3 train_uzbek.py --register colloquial \
        --dataset_path data/aziza-bilingual/aziza-uzbek.jsonl \
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Train Uzbek QLoRA adapter for Aziza/PersonaPlex-7B"
    )
    p.add_argument(
        "--register",
        choices=list(VALID_REGISTERS),
        default="colloquial",
        help="Register to train on. 'all' = use all data regardless of register.",
    )
    p.add_argument(
        "--language",
...
def validate_cyrillic_coverage(records: list[dict], field_pattern: str, threshold: float = 0.6) -> float:
    ...
    if pct < threshold:
        log.warning(
            ...
            "Dataset may not be primarily Cyrillic — check language filter."
        )
    return pct
...
def build_output_dir(output_base: str, language: str, register: str) -> Path:
    lang_prefix = "uz" if language.startswith("uz_") else language
    name = f"{lang_prefix}_{register}" if register != "all" else f"{lang_prefix}_all"
...
def train(args: argparse.Namespace) -> None:
    ...
    if "cyrillic" in args.language or args.language == "ru":
    ...
    # ── SFT training args ────────────────────────────────────────────────────
    lang_prefix = "uz" if args.language.startswith("uz_") else args.language
    run_name = f"{lang_prefix}_{args.register}_{int(time.time())}"
    ...
    log.info("Training complete. Run eval_uzbek.py before deploying.")
