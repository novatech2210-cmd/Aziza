#!/usr/bin/env python3
"""
resize_moshi_embeddings_v5.py

Fixed: saves clean state_dict (not FSDP-wrapped model) to avoid 'fsdp_best_state' error.
...
# ---------------------------------------------------------------------------
# 6. Save CLEAN state_dict (not full model to avoid FSDP issues)
# ---------------------------------------------------------------------------
print("\n[6/6] Saving clean state_dict...")
os.makedirs(OUTPUT_DIR, exist_ok=True)

# Save ONLY state_dict — this avoids FSDP wrapper metadata
clean_state_dict = moshi_lm.state_dict()
torch.save(clean_state_dict, os.path.join(OUTPUT_DIR, "model_state_dict.pt"))
print(f"    Saved: {os.path.join(OUTPUT_DIR, 'model_state_dict.pt')}")

# Also save as safetensors for compatibility
try:
    from safetensors.torch import save_file
    save_file(clean_state_dict, os.path.join(OUTPUT_DIR, "model.safetensors"))
    print(f"    Saved: {os.path.join(OUTPUT_DIR, 'model.safetensors')}")
except ImportError:
...
with open(os.path.join(OUTPUT_DIR, "resize_metadata.txt"), "w") as f:
    f.write("Embedding resize complete\n")
    f.write("="*50 + "\n")
...
print(f"\nLoad this model in training with:")
print(f"  from moshi.models import loaders, lm")
print(f"  model = loaders.get_moshi_lm('original.safetensors', device='cuda')")
print(f"  state_dict = torch.load('{OUTPUT_DIR}/model_state_dict.pt')")
