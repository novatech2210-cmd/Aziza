#!/usr/bin/env python3
"""
resize_moshi_embeddings_v3.py

Correctly resizes Moshi's text embeddings after tokenizer vocab extension.
Uses moshi.models.loaders.get_moshi_lm() which is the confirmed API in the installed package.
...
- Uzbek graphemes currently occupy IDs 32000-32014 in the extended tokenizer.
- BUT index 32000 is text_initial_token_id — we must NOT overwrite it.
- Strategy: add 1 placeholder token to consume ID 32000, shift Uzbek to 32001-32015.
- Final sizes: text_emb = 32016, text_linear = 32015.
"""
...
# ---------------------------------------------------------------------------
# 2. Load native Moshi model using get_moshi_lm
# ---------------------------------------------------------------------------
...
# Load model using the official API
...
if hasattr(moshi_lm, 'depformer_text_emb') and moshi_lm.depformer_text_emb is not None:
    ...
    for i in range(N_REAL_NEW):
        new_id = 32001 + i
        new_dep_weight[new_id] = new_emb_weight[new_id]
