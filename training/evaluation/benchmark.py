...
async def run_session(redis_url, session_id, input_pcm, start_delay=0):
    ...
    try:
        while True:
            ...
            # If we finished sending and haven't received anything for 5 seconds, break
            if send_task.done() and last_token_time and (now - last_token_time > 5.0):
                break
            
            # If we haven't received anything for 15 seconds, break
...
async def main():
    ...
    if not os.path.exists(args.audio_file):
        ...
        # 16kHz 16-bit mono sine wave, 10 seconds
        sr = 16000
    ...
    tasks = []
    # Start monitor task for 20 seconds
    monitor_task = asyncio.create_task(monitor_gpu(20))
